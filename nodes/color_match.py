...
class TSPhotoColorMatch:
    ...
    def process(self, image, ref_image):
        ...
        # Compute stats from all ref frames
        ref_frames_bgr8 = []
        for i in range(ref_np.shape[0]):
            ref_frames_bgr8.append(self._rgb01_to_bgr8(ref_np[i].astype(np.float32)))
        target_mean, target_std = self._compute_stats(ref_frames_bgr8)

        # Compute stats from all source frames
        src_frames_bgr8 = []
        for i in range(n):
            src_frames_bgr8.append(self._rgb01_to_bgr8(image_np[i].astype(np.float32)))
        source_mean, source_std = self._compute_stats(src_frames_bgr8)

        # Apply color transfer to each source frame
        out_frames_rgb01 = []
        for fr_bgr8 in src_frames_bgr8:
            rgb01 = self._apply_color_transfer_rgb01(fr_bgr8, target_mean, target_std, source_mean, source_std)
            out_frames_rgb01.append(rgb01)
